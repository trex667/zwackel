...
public class UserServiceV3 implements UserService
{
    ...
    public Result<User> findAllUsers()
    {
        ...
        TypedQuery<User> query = em.createQuery("SELECT DISTINCT u FROM UserV3 u LEFT JOIN FETCH u.addresses LEFT JOIN FETCH u.organizations", User.class);
    ...
    private void addToOrganizations(Set<UserV3> users)
    {
        String select = "SELECT o FROM OrganizationV3 o";
        TypedQuery<OrganizationV3> organizations = em.createQuery(select, OrganizationV3.class);
        if (organizations.getResultList() == null || organizations.getResultList().size() == 0)
        {
            createOrganizations(100);
        }
        organizations = em.createQuery(select, OrganizationV3.class);
        for (OrganizationV3 org : organizations.getResultList())
        {
            org.addUsers(users);
            em.merge(org);
        }
